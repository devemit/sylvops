...
These terms describe desktop interaction patterns rather than daemon domain types.

| Term | Meaning |
|---|---|
| Active session header | The session-tab strip plus the lifecycle actions for the selected session. |
| Content view | Terminal, Changes, or Details. Switching views never changes session ownership or process state. |
| Lifecycle action | An operation that changes terminal attachment or the running process, currently Open terminal, Leave terminal, and Stop. |
