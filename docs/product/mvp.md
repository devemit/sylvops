...
9. Reopen the hierarchical TUI or native desktop with the last valid context restored, inspect a bounded diff, attach in the Terminal tab, or resume.
...
The repository contains the Codex-first workflow plus release-ready usability: atomic Windows ConPTY/Job creation, protocol 1.7 workspace and entity operations, separate bounded TUI/desktop state, guided management forms, native repository selection, responsive and resizable desktop panels, keyboard and mouse workflows, embedded attachment, dynamic PTY resizing, one-command repository opening, redacted diagnostics, and release packaging. `v0.1.0` is the next ordinary application release and remains unpublished until the current tree passes the complete hosted Windows, Linux, and macOS gate.
