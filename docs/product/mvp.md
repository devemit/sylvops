...
The repository contains the Codex-first workflow plus beta usability: atomic Windows ConPTY/Job creation, protocol 1.7 workspace and entity operations, separate bounded TUI/desktop state, guided management forms, native repository selection, responsive and resizable desktop panels, keyboard and mouse workflows, embedded attachment, dynamic PTY resizing, one-command repository opening, redacted diagnostics, and release packaging. `v0.1.0-beta.1` remains unpublished until the redesigned UX passes the complete hosted Windows, Linux, and macOS gate.
