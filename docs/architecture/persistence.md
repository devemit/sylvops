...
SQLite stores metadata and bounded recent prompts, not unbounded terminal output. UUIDv7 identifiers are canonical text and timestamps are UTC Unix milliseconds. The initial migration is `crates/sylvops-daemon/migrations/0001_initial.sql`.
...
Migrations are embedded, checksum-validated, and applied before accepting IPC connections. Applied migrations are immutable. Startup refuses an unknown newer schema rather than attempting a destructive downgrade.
