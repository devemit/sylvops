...
The first client request is `Hello`. It authenticates with a high-entropy token stored in the private runtime directory and negotiates an exact protocol version before other requests are accepted. Authentication comparisons are constant-time. Protocol 1.7 includes managed-worktree controls, provider listing/probing, provider-aware session creation, Codex resume, normalized provider events, bounded Git diff requests, workspace switching, metadata-only entity renames, bounded TUI navigation persistence, and bounded native-desktop state persistence. Opcode 10 carries correlated controls and opcode 11 carries unsolicited daemon events. Snapshots and entity events carry database-actor revisions; clients detecting a revision gap request a new snapshot. UI-state writes deliberately do not advance that revision or emit lifecycle events.

The sole persisted TUI record is `client_scope = 'tui'`, `key = 'navigation.v1'`. It may contain selected project/worktree/session IDs and the selected main tab. Corrupt or stale values are ignored safely. Searches, forms, terminal input, scrollback, and secrets are never persisted there.

The desktop uses a separate `client_scope = 'desktop'`, `key = 'navigation.v1'` record. It is limited to selected IDs, the selected main tab, at most 16 open session IDs, theme, density, terminal font size, validated window size, panel ratios, and the compact navigator panel. It never stores attachment state, terminal data, form contents, searches, or credentials.
