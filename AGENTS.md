...
Windows uses an application-owned ConPTY launch with pseudoconsole and Job Object attributes applied atomically by `CreateProcessW`; Unix retains `portable-pty` plus process groups. The complete Windows/Linux/macOS suite passes for the beta candidate. Planned after beta: explicit external-worktree import, Claude, GitHub, commit/push/PR actions, history/file-finder/Git-grep tools, notifications, remote execution, and signed/native installers. UI controls and documentation must not claim planned behavior is available.
