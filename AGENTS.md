...
MVP implementation candidate: protocol 1.4, authenticated local IPC, SQLite state, repository/worktree lifecycle and reconciliation, daemon-owned plain-shell and Codex PTYs, provider probing, bounded scrollback and VT snapshots, authenticated observational Codex hooks, deterministic attention states, resume, read-only bounded diff, the four-panel Ratatui client, and an unsigned portable Windows test package. Worktree removal is non-forced, refuses tracked/untracked/ignored content, and preserves branches.

Not accepted/release-verified: execution of the complete Windows/Linux suite (including the checked-in fake-Codex hook/resume end-to-end scenario) and race-free suspended Windows spawning. Planned after MVP: explicit external-worktree import, Claude, GitHub, commit/push/PR actions, history/file-finder/Git-grep tools, notifications, remote execution, diagnostics, and packaging. UI controls and documentation must not claim planned behavior is available.
