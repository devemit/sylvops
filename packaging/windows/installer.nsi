; Vendored from cargo-packager 0.11.8 commit 9e88b74 under MIT OR Apache-2.0.
; SylvOps keeps installation files under LocalAppData\Programs, registers App Paths,
; and deliberately leaves LocalAppData\SylvOps plus roaming configuration untouched.
...
Section Install
  ...
  ; Register an OS-supported current-user CLI discovery surface without editing PATH.
  WriteRegStr HKCU "Software\Microsoft\Windows\CurrentVersion\App Paths\${MAINBINARYNAME}.exe" "" "$INSTDIR\${MAINBINARYNAME}.exe"
  WriteRegStr HKCU "Software\Microsoft\Windows\CurrentVersion\App Paths\${MAINBINARYNAME}.exe" "Path" "$INSTDIR"
...
Section Uninstall
  ...
  ; Remove installer-owned product state only when it still describes this installation.
  ...
  ; Remove App Paths only when it still points at this installation.
  ReadRegStr $R7 HKCU "Software\Microsoft\Windows\CurrentVersion\App Paths\${MAINBINARYNAME}.exe" ""
  ${If} $R7 == "$INSTDIR\${MAINBINARYNAME}.exe"
    DeleteRegKey HKCU "Software\Microsoft\Windows\CurrentVersion\App Paths\${MAINBINARYNAME}.exe"
  ${EndIf}
